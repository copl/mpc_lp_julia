...
type matlab_ldl
	...
	function matlab_ldl(sparse_matrix::SparseMatrixCSC{Float64,Int64}, num_vars::Int64, num_constraints::Int64)
		...
		this.factorize = function()#sigma::Float64 = 0.0)
			try 
				@matlab begin
					mat_L, mat_D, mat_P, mat_S = ldl(sparse_matrix);
					diag_vec = diag(mat_D);
				end

				@mget diag_vec
				this.diag_vec = full(diag_vec)[:]
			catch e
		...
		this.solve = function(rhs::Array{Float64,1})
			try 
				@mput rhs
				#mat_solution = lsqlin([sparse_matrix; 0.4*speye(num_vars, num_vars + num_constraints)], [rhs; zeros(num_vars,1)]);
				@matlab begin
					mat_solution = mat_S * ( mat_P* (mat_L'\ (mat_D\ (mat_L\ (mat_P' * ( mat_S * rhs ) ) ) ) ) );
				end
			
				@mget mat_solution
				sol::Array{Float64,1}
				sol = full(mat_solution)[:];
				@assert(all(~isnan(sol)))

				return sol
			catch e
				println("ERROR matlab_ldl.solve")
				throw(e)
			end
		end

		this.calc_min_mod = function()
			mval = 5.0;
			@mput mval 
			eval_string("for sigma = [0, 10.^(linspace(-5,mval,200))]
					sigma;
					[mat_L, mat_D, mat_P, mat_S] = ldl(sparse_matrix + sigma*sparse(diag([ones(num_vars,1); zeros(num_constraints,1)])));
					diag_vec = diag(mat_D);
			...
			@mget sigma
			if (sigma == 10^mval)
				@mget pos_eigs neg_eigs
				println("target:",num_vars, " ", num_constraints)
				println("actual",pos_eigs, " ", neg_eigs)
				@assert(sigma != 10^mval)
			end
			println("sigma for convexity = ",sigma)
			return sigma
		end
		
		try
			num_vars = num_vars + 0.0;
			num_constraints = num_constraints + 0.0;
			@mput num_vars num_constraints

	   		n = num_constraints + num_vars;
			@assert(size(sparse_matrix) == (n,n));
			@mput sparse_matrix;
			this.factorize();

			return this;
		catch	e
			println("ERROR matlab_ldl.matlab_ldl")
			throw(e)
		end
	end
end
...
type class_direction
	...
	function class_direction()
		...
		this.update_factorization = function(local_approx::class_local_approximation, nlp::class_non_linear_program, vars::class_variables, settings::class_settings)
			...
			try
				this.K_bar = sparse(
						[[local_approx.D_x local_approx.J']
						[local_approx.J -local_approx.D_z]]
					);
				
								
				#this.factored_K_bar = lufact(K_bar);
				n = size(local_approx.D_x,1);
				m = size(local_approx.D_z,1);
				this.factored_K_bar = matlab_ldl(this.K_bar,n,m);
		...
		this.compute_p_vector = function(local_approximation,variables)
			try
				this.p = this.factored_K_bar.solve([local_approximation.v2; local_approximation.v3]);
		...
		this.compute_direction = function(local_approximation::class_local_approximation,vars::class_variables)
			try
				...
				q = this.factored_K_bar.solve(r_hat); # (35)
				...
				@assert(~isnan(this.dtau))
				@assert(~isinf(this.dtau))				

				direction_vector = q - this.p * (this.dtau); # ERROR IN YINYU ANDERSON 1
				@assert(~all(isnan(direction_vector)))				
