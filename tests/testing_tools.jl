...
function standard_settings()
	...
	settings.max_iter = 150;  # Total number of iterarions
	settings.min_alpha = 1e-3;
	
	settings.kkt_feasible = 1e-6
	...
	settings.beta1 = 10.0^(-4) #10.0^(-4)
	settings.beta2 = 10.0^(-3) #10.0^(-8)
...
function solve_with_JuMP(A, b, c, solver=IpoptSolver(max_iter=300)) #Output=0
	...
	#@setNLObjective( model, Min, sum{ c[j]*x[j] - 0.5*1e-3*x[j]*x[j], j=1:k} )
	@setNLObjective( model, Min, sum{ c[j]*x[j] , j=1:k} + sum{x[j]*x[j+1], j=1:k-1} )
...
function solve_net_lib_problem(A,b,c,settings)
	...
	#lp.set_quadratic_objective(c,-1e-3*speye(length(c)));
	Q = spzeros(k,k)
	for j = 1:(k-1)
		Q[j,j+1] = 2.0	
		Q[j+1,j] = 2.0	
