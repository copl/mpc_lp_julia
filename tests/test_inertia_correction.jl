...
function main()
	...
	if true
		#trivial_non_convex_test(settings,[-9.0],3)
		#non_convex_test(settings,[1.0,-10.0],[[1.0 1.0]],[1.0],1)		
		#non_convex_test(settings,[1.0,-10.0],[[1.0 1.0]],[-1.0],2)
		non_convex_test(settings,[1.0,-10.0],[[1.0 -1.0]],[0.0],3)
	end
...
function trivial_non_convex_test(settings,quad,correct_status)
	try
		A = spzeros(0,1);
		b = EMPTY_ARRAY;
		...
		qp.set_quadratic_objective([0.0],spdiagm(quad));
		qp.set_linear_constraints(sparse(A),b,sparse(A),b,1);
		...
		is_problem_successful("trivial_non_convex_quadratic_problem",status,correct_status,details)
		println(vars.x_scaled())
	catch e
		println (e)
	end
end

function non_convex_test(settings,quad,A_bar,b_bar,correct_status)
	try
		A = spzeros(0,2);
		b = EMPTY_ARRAY;
		...
		qp.set_quadratic_objective([0.0, 0.0],spdiagm(quad));
		qp.set_linear_constraints(sparse(A),b,sparse(A_bar),b_bar,2);
		...
		is_problem_successful("non_convex_quadratic_problem",status,correct_status,details)
		println(vars.x_scaled())
	catch e
		println (e)
	end
...
function trival_non_convex_quadratic_problem1(settings)
	...
	qp.set_quadratic_objective([1.0, 2.0],-speye(2));
	qp.set_linear_constraints(sparse(A),b,sparse(A_bar),b_bar,2);
...
function trival_non_convex_quadratic_problem4(settings)
	...
	qp.set_quadratic_objective([-3.0],sparse(-ones(1,1)));
	qp.set_linear_constraints(sparse(A),b,sparse(A_bar),b_bar,1);
	...
	details = "min -3*x-10*x^2" * "\n" * "s.t. x >= 0";
	
	is_problem_successful("trival_non_convex_quadratic_problem4",status,3,details)
end
